...
def test_complete_employee_management_workflow(test_client, sample_excel_file):
    # Upload Excel file
    response = test_client.post("/api/session/upload", files={"file": sample_excel_file})
    ...
    # Move an employee
    response = test_client.put("/api/employees/1/move", json={"grid_position": 5})
    assert response.status_code == 200
...
```python
def create_employee(
    id: int = 1, name: str = "Test Employee", performance: int = 3, potential: int = 3, **kwargs
) -> Employee:
