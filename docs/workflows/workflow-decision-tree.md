...
```mermaid
graph TD
    Start[What do you need to do?] --> Prep{Preparing for<br/>a meeting?}
    Prep -->|Yes| Calibration[Talent Calibration Workflow]
    Prep -->|No| Change{Need to move<br/>employees or update<br/>ratings?}
    Change -->|Yes| Making[Making Changes Workflow]
    Change -->|No| Notes{Adding documentation<br/>or notes?}
    Notes -->|Yes| AddNotes[Adding Notes Workflow]
    Notes -->|No| Learn{Learning the<br/>system?}
    Learn -->|Yes| Quick[Start with Quickstart]
    Learn -->|No| Feature[Browse Feature Guides]

    click Calibration "talent-calibration.html"
    click Making "making-changes.html"
    click AddNotes "adding-notes.html"
    click Quick "../quickstart.html"
```
