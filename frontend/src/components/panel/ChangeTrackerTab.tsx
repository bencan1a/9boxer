  ...
  const renderChangesTable = (
    ...
    return (
      <Table
        ...
        sx={{ tableLayout: "auto", minWidth: 400 }}
      >
        <TableHead>
          <TableRow>
        ...
        <TableBody>
          {changesList.map((change) => (
            <React.Fragment key={`${change.employee_id}-${change.timestamp}`}>
              ...
              <TableRow
                data-testid={`${testIdPrefix}-notes-row-${change.employee_id}`}
              >
                <TableCell colSpan={2} sx={{ pt: 0, pb: 2 }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                    ...
                    <TextField
                      ...
                      size="small"
                      placeholder={t(
                        "panel.changeTrackerTab.addNotesPlaceholder"
                      )}
                      value={
                        editingId === change.employee_id
                          ? notesValue
                          : change.notes || ""
                      }
                      onFocus={() =>
                        handleNotesFocus(change.employee_id, change.notes)
                      }
                      onChange={handleNotesChange}
                      onBlur={() =>
                        handleNotesBlur(change.employee_id, isDonut)
                      }
                      multiline
                      maxRows={3}
        ...
        >
          ...
          <Box sx={{ flex: 1, overflowX: "auto", overflowY: "auto", mt: 2 }}>
            <Paper variant="outlined">
              ...
              {activeTab === 1 &&
                renderChangesTable(sortedDonutChanges, true, "donut-change")}
            </Paper>
          </Box>
        </Box>
      ) : (
