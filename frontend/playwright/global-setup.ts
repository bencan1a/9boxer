...
async function waitForBackend(): Promise<void> {
  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    try {
      ...
      if (i % 5 === 0 && i > 0) {
        console.log(`[Global Setup] Still waiting for backend... (${i}s elapsed)`);
      }
...
async function startBackend(): Promise<ChildProcess> {
  ...
  const backend = spawn(
    pythonPath,
    ['-m', 'uvicorn', 'ninebox.main:app', '--host', '127.0.0.1', '--port', '8000'],
    {
      cwd: backendDir,
  ...
  backend.on('exit', (code, signal) => {
    if (code !== null && code !== 0) {
      ...
      console.log(`[Global Setup] Backend process killed with signal ${signal}`);
    }
  });
