...
test.describe('Employee Details Panel Flow', () => {
  ...
  test('should display employee details when clicking employee card', async ({ page }) => {
    ...
    const detailsPanel = page.locator('[role="tabpanel"]').filter({ hasText: 'Employee Information' });
    ...
    const hasManagementData = await detailsPanel.getByText('No management chain data available').isVisible().catch(() => false);

    if (!hasManagementData) {
      // If management chain data exists, verify employee appears
      const chainSection = detailsPanel.locator('text=Reporting Chain').locator('../..');
      await expect(chainSection).toContainText('Alice Smith');
    } else {
      // If no data, just verify the empty state message appears
      await expect(detailsPanel.getByText('No management chain data available')).toBeVisible();
    }
  });
  ...
  test('should display movement timeline in details tab', async ({ page }) => {
    ...
    const detailsPanel = page.locator('[role="tabpanel"]').filter({ hasText: 'Performance History' });
    ...
    await expect(detailsPanel.getByText('Current Assessment').first()).toBeVisible();
    ...
    const updatedDetailsPanel = page.locator('[role="tabpanel"]').filter({ hasText: 'Performance History' });
    await expect(updatedDetailsPanel.getByText('Performance History')).toBeVisible();
    ...
    await expect(updatedDetailsPanel.getByText('Current Assessment').first()).toBeVisible();
  ...
  test('should close details panel when clicking outside', async ({ page }) => {
    ...
    const updatedDetailsPanel = page.locator('[role="tabpanel"]').filter({ hasText: 'Employee Information' });
    await expect(updatedDetailsPanel.getByRole('heading', { name: 'Alice Smith' })).toBeVisible();
  });
  ...
  test('should show employee information section', async ({ page }) => {
    ...
    const jobFunctionExists = await detailsPanel.getByText('Job Function').isVisible().catch(() => false);
  ...
  test('should display promotion readiness checkbox', async ({ page }) => {
    ...
    const promotionCheckbox = detailsPanel.getByRole('checkbox', { name: /Promotion Ready/i });
  ...
  test('should display potential chip with correct styling', async ({ page }) => {
    // Click employee card
    const employeeCard = page.locator('[data-testid="employee-card-1"]');
    await employeeCard.click();
    ...
    const potentialSection = detailsPanel.locator('text=Potential:').first().locator('..');
    await expect(potentialSection).toBeVisible();
  });

  test('should display position label in current assessment', async ({ page }) => {
    // Click employee card
    const employeeCard = page.locator('[data-testid="employee-card-1"]');
    await employeeCard.click();
    ...
    const positionSection = detailsPanel.locator('text=Position:').locator('..');
    await expect(positionSection).toBeVisible();
  });

  test('should show modified indicator when employee is moved', async ({ page }) => {
  ...
  test('should display multiple management chain levels', async ({ page }) => {
    ...
    const detailsPanel = page.locator('[role="tabpanel"]').filter({ hasText: 'Reporting Chain' });
    await expect(detailsPanel.getByText('Reporting Chain')).toBeVisible();
    ...
    if (!hasManagementData) {
      ...
      const chainContent = detailsPanel.locator('text=Reporting Chain').locator('../..');
  ...
  test('should persist employee selection when switching tabs', async ({ page }) => {
    // Click employee card
    const employeeCard = page.locator('[data-testid="employee-card-1"]');
    await employeeCard.click();
