...
test.describe('Statistics Tab Display', () => {
  ...
  test('should display statistics tab with distribution data', async ({ page }) => {
  ...
  test('should show correct employee counts in distribution', async ({ page }) => {
    ...
    const totalEmployeesCard = page.locator('text=Total Employees').locator('..').locator('..');
    const totalEmployeesText = await totalEmployeesCard.locator('h4').textContent();
  ...
  test('should update statistics after employee movement', async ({ page }) => {
    ...
    const modifiedEmployeesCard = page.locator('text=Modified').locator('..').locator('..');
    const modifiedEmployeesText = await modifiedEmployeesCard.locator('h4').textContent();
    const modifiedEmployees = parseInt(modifiedEmployeesText || '0');
    expect(modifiedEmployees).toBe(1);
  });

  test('should display grouped statistics with correct percentages', async ({ page }) => {
    // Click Statistics tab
    await page.locator('[data-testid="statistics-tab"]').click();
  ...
  test('should show zero counts for empty boxes', async ({ page }) => {
    ...
    const box5Row = page.locator('tr', { has: page.getByRole('rowheader', { name: 'Core Talent [M,M]' }) });
