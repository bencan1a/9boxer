...
test.describe('Employee Upload Flow', () => {
  ...
  test('should upload Excel file and view employees in the grid', async ({ page }) => {
    ...
    await expect(page.getByText('sample-employees.xlsx')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="grid-box-9"]')).toContainText('Alice Smith');
  });
  ...
  test('should display employee count in grid boxes after upload', async ({ page }) => {
