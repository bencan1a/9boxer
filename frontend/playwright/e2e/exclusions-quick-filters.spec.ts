...
test.describe("Employee Exclusions with Quick Filters Flow", () => {
  ...
  test("should open manage exclusions dialog", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="exclude-vps-button"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="exclude-directors-plus-button"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="exclude-managers-button"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="clear-selections-button"]')
    ).toBeVisible();

    // Verify employee list is shown
    const employeeCheckboxes = page.locator(
      '[data-testid^="exclusion-checkbox-"]'
    );
    const checkboxCount = await employeeCheckboxes.count();
    expect(checkboxCount).toBeGreaterThan(0);
  });

  test("should exclude employees using quick filter button", async ({
    page,
  }) => {
    // Get initial employee count from the app bar
    const employeeCountText = await page
      .locator('[data-testid="employee-count"]')
      .textContent();
    const initialCountMatch = employeeCountText?.match(
    ...
    const newCountText = await page
      .locator('[data-testid="employee-count"]')
      .textContent();
    const newCountMatch = newCountText?.match(
      /(\d+)\s+of\s+(\d+)\s+employees?/i
    );
  ...
  test("should show excluded employees as checked when reopening dialog", async ({
    page,
  }) => {
    ...
    const reopenedCountText = await page
      .locator('[data-testid="selected-count"]')
      .textContent();
    const reopenedCount = parseInt(
      reopenedCountText?.match(/(\d+)/)?.[1] || "0"
    );
  ...
  test("should restore excluded employees when using clear selections", async ({
    page,
  }) => {
    // Get initial count
    const initialCountText = await page
      .locator('[data-testid="employee-count"]')
      .textContent();
    const initialCountMatch = initialCountText?.match(
      /(\d+)\s+(?:of\s+)?(\d+)?\s*employees?/i
    );
    const initialCount = initialCountMatch ? parseInt(initialCountMatch[1]) : 0;
    ...
    const excludedCountText = await page
      .locator('[data-testid="employee-count"]')
      .textContent();
    const excludedCountMatch = excludedCountText?.match(
      /(\d+)\s+(?:of\s+)?(\d+)?\s*employees?/i
    );
    const excludedCount = excludedCountMatch
      ? parseInt(excludedCountMatch[1])
      : initialCount;
    ...
    const restoredCountText = await page
      .locator('[data-testid="employee-count"]')
      .textContent();
    const restoredCountMatch = restoredCountText?.match(
      /(\d+)\s+(?:of\s+)?(\d+)?\s*employees?/i
    );
    const restoredCount = restoredCountMatch
      ? parseInt(restoredCountMatch[1])
      : 0;
  ...
  test("should exclude Directors+ using quick filter", async ({ page }) => {
    ...
    if (selectedCount > 0) {
      ...
      await expect(
        page.locator('[data-testid="employee-count"]')
      ).toContainText(/\d+\s+of\s+\d+\s+employees?/i);
    }
  });
