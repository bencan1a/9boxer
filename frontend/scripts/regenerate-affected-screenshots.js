...
function loadAffectedScreenshots() {
  ...
  if (!fs.existsSync(dataPath)) {
    console.error("❌ .affected-screenshots.json not found!");
  ...
  try {
    ...
    console.error(
      "❌ Failed to load .affected-screenshots.json:",
      error.message
    );
    process.exit(1);
  }
...
function regenerateScreenshot(screenshotId, dryRun = false) {
  ...
  try {
    ...
    console.log(`   ✅ Success: ${screenshotId}`);
    return true;
  } catch (error) {
 ...
 */
function main() {
  console.log("📸 Regenerating affected documentation screenshots...\n");

  // Check for dry-run flag
  const dryRun = process.argv.includes("--dry-run");
  if (dryRun) {
    console.log(
      "🏃 DRY RUN MODE - No screenshots will be actually regenerated\n"
    );
  }
  ...
  if (data.metadata.themeChanged) {
    console.log(`   ⚠️  Theme changed: Consider regenerating ALL screenshots`);
  }

  console.log();

  if (data.screenshots.length === 0) {
    console.log("✅ No screenshots to regenerate");
    process.exit(0);
  }

  console.log(`📸 Regenerating ${data.screenshots.length} screenshot(s):\n`);
  ...
  if (dryRun) {
    ...
    console.log("\n⚠️  Some screenshots failed to regenerate");
    process.exit(1);
  } else {
