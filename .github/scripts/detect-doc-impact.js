...
function loadComponentScreenshotMap() {
  ...
  if (!fs.existsSync(mapPath)) {
    console.error('❌ component-screenshot-map.json not found!');
  ...
  try {
    ...
    console.error('❌ Failed to load component-screenshot-map.json:', error.message);
    process.exit(1);
  }
}
...
function analyzeDocumentationImpact(changedFiles, mapping) {
  ...
  console.log(`\n📁 Changed frontend files: ${frontendFiles.length}`);
  ...
  for (const file of frontendFiles) {
    ...
    if (mapping.componentToScreenshots[relativePath]) {
      ...
      console.log(`   ✅ Found in mapping: ${componentData.component}`);
      console.log(`   📸 Affects ${screenshots.length} screenshot(s): ${screenshots.join(', ')}`);
      ...
      console.log(`   ⏭️  Not in component mapping (no screenshots)`);
    }
    ...
    if (dependentComponents.length > 0) {
      console.log(`   🔗 Affects ${dependentComponents.length} dependent component(s)`);
  ...
  if (themeChanged) {
    console.log('\n🎨 Theme files changed - potentially affects all screenshots');
  }
...
function saveAffectedScreenshots(results) {
  ...
  try {
    ...
    console.log(`\n💾 Saved affected screenshots to: ${path.relative(PROJECT_ROOT, outputPath)}`);
  } catch (error) {
    console.error('❌ Failed to save affected screenshots:', error.message);
    process.exit(1);
  }
}
...
function printSummary(results) {
  console.log('\n' + '─'.repeat(60));
  console.log('📊 Documentation Impact Analysis Summary');
  console.log('─'.repeat(60));

  console.log(`\n📁 Changed Files: ${results.changedFiles.length}`);
  console.log(`🧩 Changed Components: ${results.changedComponents.length}`);
  console.log(`📸 Affected Screenshots: ${results.affectedScreenshots.length}`);

  if (results.themeChanged) {
    console.log(`🎨 Theme Changed: YES (may require broader screenshot review)`);
  }

  if (results.changedComponents.length > 0) {
    console.log('\n📋 Component Details:');
    results.componentDetails.forEach((comp) => {
      const badge = comp.indirect ? '🔗' : '✅';
  ...
  if (results.affectedScreenshots.length > 0) {
    ...
    console.log('\n✅ No screenshots affected by these changes');
  }
 ...
 */
function main() {
  console.log('🔍 Analyzing documentation impact from code changes...\n');
  ...
  console.log(`📦 Loaded mapping with ${mapping.metadata.totalComponents} components`);
